...
public class SametimeBot4multiChat implements ServiceListener, LoginListener, ImServiceListener, ImListener {
	...
	PlaceMemberAdapter placeMemberAdapter;
	
	InvitationManager invManage;
	...
	public SametimeBot4multiChat(String sessionName) throws DuplicateObjectException {
	...
	public void loggedIn(LoginEvent event) {
		...
		invManage.setListener(new InvitationListener() {

			@Override
			public void invitedToMeeting(Invitation arg0) {
				System.out.println("invitedToMeeting(Invitation arg0) ");
			}
			
		});

		placesService = (PlacesService) session.getCompApi(PlacesService.COMP_NAME);
		placesService.addPlacesServiceListener(new PlacesServiceListener() {

			@Override
			public void serviceAvailable(PlacesServiceEvent event) {
				System.out.println("PlacesServiceListener#serviceAvailable(PlacesServiceEvent event)");
			...
			public void serviceUnavailable(PlacesServiceEvent event) {
				System.out.println("PlacesServiceListener#serviceUnavailable(PlacesServiceEvent event)");
				
			}
			
		});
		
		placesComp = (PlacesComp) session.getCompApi(PlacesComp.COMP_NAME);
		placesComp.addPlacesServiceListener(new PlacesServiceListener() {
			
			@Override
			public void serviceUnavailable(PlacesServiceEvent arg0) {
				System.out.println("PlacesServiceListener#serviceUnavailable(PlacesServiceEvent arg0)");
			}
			
			@Override
			public void serviceAvailable(PlacesServiceEvent arg0) {
				System.out.println("PlacesServiceListener#serviceAvailable(PlacesServiceEvent arg0)");
	...
	private void debugImEvent(ImEvent event) {
		...
		if (null != bytes) {
			StringBuffer buf = new StringBuffer("Data is ");
			for (byte b : bytes) {
				buf.append(String.format(" %X", b));
			}
			printConsole(buf.toString());
			
			buf = new StringBuffer("Data is ");
			for (byte b : bytes) {
				
				buf.append(" ").append(new Character((char) b));
			}
			printConsole(buf.toString());
			...
			try {
				...
				buf = new StringBuffer("messageBytes is ");
